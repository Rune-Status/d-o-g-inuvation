...
import org.objectweb.casm.commons.cfg.tree.Tree;
...
import static org.objectweb.casm.tree.AbstractInsnNode.*;
...
public class AbstractNode extends Tree<AbstractNode> implements Opcodes {
    ...
    public <T extends AbstractNode> List<T> findChildren(Class<T> type) {
    ...
    public List<NumberNode> findNumericChildren() {
        return findChildren(NumberNode.class);
    }

    public <T extends AbstractNode> List<T> layerAll(Class<T>... opcodes) {
        List<T> children = findChildren(opcodes[0]);
        if (children == null) return null;
        if (opcodes.length == 1) return children;
        for (int i = 1; i < opcodes.length; i++) {
            List<T> next = new ArrayList<>();
            for (AbstractNode n : children) {
                List<T> match = n.findChildren(opcodes[i]);
    ...
    public boolean hasChild(Filter<AbstractNode> filter) {
