package org.objectweb.casm.commons.wrapper;
...
public class ClassFactory {
    ...
    public int fieldCount(String desc, boolean includeStatic) {
        return findFields(f -> {
            if (f.local() || (!f.local() && includeStatic)) {
                return desc == null || f.desc().equals(desc);
            }
            return false;
        }).size();
    }
    ...
    public int abnormalFieldCount(boolean includeStatic) {
        return findFields(f -> {
            if (f.local() || (!f.local() && includeStatic))
                return f.desc().contains("L") && f.desc().endsWith(";") && !f.desc().contains("java");
            return false;
        }).size();
    }
    ...
    public int methodCount(String desc, boolean includeStatic) {
        return findMethods(m -> {
            if (m.local() || (!m.local() && includeStatic)) {
                return desc == null || m.desc().equals(desc);
            }
            return false;
        }).size();
    }
